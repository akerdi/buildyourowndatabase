...
B-tree 是 SQLite 用于构建原始数据和索引(indexes)的数据结构，所以他是相当核心的点。这个章节是用于介绍这个数据结构的。
...
那个拥有子节点的 Nodes 我们称之为"内部"(Internal)节点。内部节点和叶子(Leaf)节点构造是不同的:
...
当我们实现这个数据结构，每个节点相当于一个页。根(root)节点保留为 page 0 的节点坐标。子 pointers 会简单地记录页面号码(number)，而对应页号中保存真实数据。
